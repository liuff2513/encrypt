...
import com.baihui.core.encrypt.util.ClassUtil;
...
public class EncryptClassLoader extends WebappClassLoader {
	...
	private byte[] loadClassBytesEncrypt(String name) throws IOException {
		...
		try {
			...
			int ch;
			while ((ch = in.read()) != -1) {
				buffer.write(ClassUtil.decrypt(ch));
			}
			in.close();
	...
	private void readProperties() {
		InputStream in = EncryptClassLoader.class.getResourceAsStream("loader.properties");
